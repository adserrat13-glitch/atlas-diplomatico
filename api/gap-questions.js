...
/* ─── generate (ex generate-gap-questions.js) ──────────────────────────── */
...
/* ─── evaluate (ex evaluate-gap-answer.js) ─────────────────────────────── */

const SYSTEM_PROMPT_EVAL = `Você é um examinador do CACD avaliando se o candidato realmente domina o conceito testado.
...
module.exports = async function handler(req, res) {
  ...
  const body = req.body || {};
  const isEvaluate = typeof body.pergunta === 'string' && typeof body.resposta === 'string';

  if (isEvaluate) return handleEvaluate(req, res, apiKey);
